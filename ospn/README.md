...
1. query関数でKVMサーバの以下リソース状況を取得する  
  ...
  1-2. 仮想CPUの数  
  1-3. 物理メモリのサイズ  
  1-4. 仮想マシンへ割り当てたメモリのサイズ  
  1.5. 物理ディスクのサイズ  
  1-6. 仮想マシンへ割り当てたボリュームのサイズ  
  1-7. KVMサーバ上に、すでに存在する仮想マシン名  
2. test関数でKVMのリソースに空きがあるか以下の式で判定する  
  1-1. 仮想CPU数 + これから作成する仮想マシンのCPU数 < 物理CPUの数  
  1-2. 仮想マシンのメモリ合計 + これから作成する仮想マシンのメモリ < 物理メモリのサイズ  
  1-3. 仮想マシンのボリューム合計 + これから作成する仮想マシンのボリューム < 物理ディスクのサイズ  
  1-4. これから作成する仮想マシンの名前が、KVMサーバ上の仮想マシンと重複しない  
3. 仮想マシンを作成する  
...
(1)すでに存在する仮想マシンのCPUの数 + (2)新しく作成する仮想マシンのCPU < (3)物理CPUの数

(1)(2)(3)のそれぞれを取得するShellScriptを書くと、こんな感じです
